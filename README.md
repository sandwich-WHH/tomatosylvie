...
莫兰迪克制 × HEYTEA 艺术家拙趣 × 手写字体温度（v2.2）

- 纯白背景（`#FFFFFF`）+ 鼠尾草绿主色（`#7E9280`）+ 苔藓 FAB（`#2B2826` 墨黑）
- 6 种心情采用莫兰迪低饱和色（灰绿/灰蓝/灰紫/蓝灰/灰赭/灰陶），配透明 PNG 裸插画（真实物 + 简笔小人）
- 时间线式首页：左侧日期节点 + 竖线，卡片内仅显示时间与心情符号
- 顶部周视图横条，有记录的日子显示当天心情
- 大量留白，低饱和配色，减少倾诉心理压力
- 完整设计系统（色彩/字体/组件/插画提示词/抠图脚本）见 [`design/`](design/design-spec.md)，可交互预览见 [`design/assets/preview-template.html`](design/assets/preview-template.html)
