...
```swift
enum AppTheme {
    static let bgBeige   = Color(hex: 0xF7F1E5)
    static let cardCream = Color(hex: 0xFFFDF7)
    static let matcha    = Color(hex: 0x8A9B6E)
    static let leafDark  = Color(hex: 0x5C6B46)
    static let ink       = Color(hex: 0x3A3A32)
    static let sketch    = Color(hex: 0x6B6657)
}
```
- 禁止在 View 内硬编码色值/字号，一律取 Token。
- 手写感：标题用系统手写类字体 fallback + 版式/描边营造；正文系统字体。
